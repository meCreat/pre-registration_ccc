...
<div class="container row" style="border: 1px solid ;border-radius: 20px; padding: 20px 5px 5px 5px; margin-bottom: 20px;">
                ...
                <div class="col-md-4">
                        <span style="float: right;" id="timestamp"><?php  echo "Today is&nbsp;" . date("l") . "&nbsp;" . date("M d Y h:i a"); ?></span>
                </div>
        ...
        <?php 
        $todaysReg = $conn->query("SELECT id_number FROM student_data WHERE archive='0' AND status='registered' AND `reg-date` BETWEEN '".date("Y-m-d 00:00:00")."' AND '".date("Y-m-d 23:59:59")."' ");
      
         ?>
        <div class="col-sm-4">
          <?php   if ($dateStart <= time() && $dateEnd >= time()) :?>
          <div class="alert alert-success "><span > Todays Registered: <?= $todaysReg->num_rows?></span></div>
          <?php 
        endif; 
        ?>
        </div>

        <div class="col-sm-4"><span><?=$reg_status?></span></div>
        </div>
