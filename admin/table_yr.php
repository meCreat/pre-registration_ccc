<?php
$i = 0; 
$retVal = (isset($_GET['y_lvl'])) ? trim($_GET['y_lvl']) : 0 ;
if($retVal == 1){
  $head = 'Freshmens';
  $history = '1st Year';
}else if($retVal == 2){
...
<div class="row">
    ...
    <div class="col-md-2">
      <span>Status: </span>
    <form id="submit_reg" action="table_yrRegistered" method='POST'>
      <select id="reg" name="reg">
        <option selected='' disabled=''><?php $retVal = (isset($_GET['reg'])) ? ucfirst($_GET['reg']) : 'Select Status'; echo  $retVal;?></option>
        <option value="0">All</option>
        <option value="registered">Registered</option>
        <option value="unregistered">Unregistered</option>
      </select>
        <input type="hidden" name="head" value="<?=$head?>">
        <input type="hidden" name="link" value="<?=$_GET['y_lvl']?>">
...
<div style="border:1px solid; margin:20px; padding:10px; border-radius:10px; background-color:    hsl(180, 100%, 99%);">
  ...
  <div class='' style="margin-right: 20px; margin-left: 20px; padding-top: 20px;">
    <form id="select" action="selected_st?y_lvl=<?=$_GET['y_lvl']?>" method="POST" target="_blank">
