...
if (isset($_GET['dept'])) {
  $code = 'AND program_dept.dept_id ='.mysqli_real_escape_string($conn, $_GET['dept']);
  $head = $_GET['head'];
  $link .= "&dept=".$_GET['dept'];
}

if (isset($_GET['course'])) {
  $code = 'AND courses.id ='.mysqli_real_escape_string($conn, $_GET['course']);
  $head = $_GET['head'];
  $link .= "&course=".$_GET['course'];
}
...
$head1 = '';
if(isset($_GET['year'])){
  $code1 = 'AND year_lvl ='.mysqli_real_escape_string($conn, $_GET['year']);
  $excelLink1 = '&y_lvl='.$_GET['year'];
  $head1 = ' Year Level '.$_GET['year'].' ';
}

if(isset($_GET['reg'])){
  $excelLink2 = '&reg='.$_GET['reg'];
  $code2 = "AND student_data.status ='".mysqli_real_escape_string($conn, $_GET['reg'])."'";
}
?>
...
<div class="container" >
  <div style="border-bottom: 1px solid grey;margin-bottom: 5px;" >
    ...
    $Dlink = '';
    if(!isset($_GET['abbC'])){
      $here = "All ".mysqli_real_escape_string($conn, $_GET['abbD'])." Courses";
    }else{
      $here = $_GET['abbC'];
    }
    ?>
    <span class="p-2">Dashboard/ Tables/ <?=$_GET['abbD']?> Table/ <?=$here?></span>  
  </div>
</div>

<div class="container">
    <div  class="col-md-12">
      <h2><?=$head?> <?=$head1?>Table</h2>
...
<div class="row">
    <div class="col-md-4">
      ...
      <a id="excelBtn" href="excel_table_gen.php?head=<?=$head.$link?><?=$excelLink1.$excelLink2?>" class="btn btn-success"><i class="bi bi-file-earmark-spreadsheet-fill"></i> Excel</a>

    </div>

    <div class="col-md-8">
    <div class="float-right">
      <span>Year Level:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Status:</span>

    <form id="submit_y" action="table_dept&courseYear?abbD=<?php echo $_GET['abbD'];?>" method='POST'>
      <select id="year" name="year">
        <option selected='' <?php if(!isset($_GET['year'])){ echo "disabled=''"; } ?> value="<?php if(isset($_GET['year'])){ echo $_GET["year"]; }?>"><?php if(isset($_GET['year'])){ echo $_GET["year"]; }else echo 'Select Year'; ?></option>
        ...
        <select name="reg">
        <option selected='' <?php if(!isset($_GET['reg'])) { echo "disabled=''"; } ?>><?php if(isset($_GET['reg'])){ echo ucfirst($_GET['reg']); }else echo 'Select Status';?></option>
    ...
    <script type="text/javascript">
      $(document).ready(function(){
        $('#year, [name=reg]').on('change',function(){
          console.log('ss');
          $('#submit_y').submit();
        })
      })
...
<div style="border:1px solid; margin:20px; padding:10px; border-radius:10px; background-color:    hsl(180, 100%, 99%);">
  ...
  <div class="btn-group" id="btn-table">
    ...
    <div  class="dropdown-menu">
      ...
      <button id="btn_ra" class="btn btn-primary dropdown-item"  data-toggle="modal" data-target="#archive_selected_st"><i class="bi bi-file-pdf-fill"></i> Move to Archive</button>    
    </div>
  </div>
  </div>
    ...
    <div style="width: 5080px;" class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
            <button form="select" type="submit" name="submit_remove" class="btn btn-primary">Yes</button>
        ...
        <div class="modal-body">
          <div class="container">
            <form id="submit" action="./fpdf/table_general_pdf" method="POST" target='_blank'>
              ...
              <input type="hidden" name="year_lvl" value="<?php if(isset($_GET['year'])){ echo $_GET['year']; }else{ echo '0'; } ?>">
               <br>
